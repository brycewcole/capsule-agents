...
class ConfigureService:
    def __init__(self, db_url: Annotated[str, Depends(database_url)]):
        ...
        logger.debug("Checking for 'tools' column in 'agent_info' table.")
        cursor = conn.execute("PRAGMA table_info(agent_info)")
        columns = [column[1] for column in cursor.fetchall()]

        # Create the table if it doesn't exist
        logger.debug("Ensuring 'agent_info' table exists.")
        conn.execute("""
        ...
        if "tools" not in columns:
            ...
            logger.debug("'tools' column already exists in 'agent_info' table.")

        conn.commit()
        # insert mock data if it doesn't exist yet
        logger.debug("Checking for existing agent_info data with key = 1.")
        row = conn.execute("SELECT 1 FROM agent_info WHERE key = 1").fetchone()
        if not row:
            logger.info("No existing agent_info data found with key = 1, inserting mock data.")
            mock = AgentInfo(
                name="capy_agent",
    ...
    def _get_conn(self):
        ...
        logger.debug(f"Database connection established to {self.db_url}")
        return conn
    ...
    def upsert_agent_info(self, info: AgentInfo) -> AgentInfo:
        ...
        for tool_config in info.tools:
            if tool_config.type == "a2a_call":
                logger.debug(f"Validating a2a_call tool: {tool_config.name}")
                if not isinstance(tool_config.tool_schema, dict):
                    logger.error(f"Validation failed for tool '{tool_config.name}': tool_schema is not a dict.")
                    raise ValueError(
                ...
                if "agent_url" not in tool_config.tool_schema:
                    logger.error(f"Validation failed for tool '{tool_config.name}': 'agent_url' missing in tool_schema.")
                    raise ValueError(
                        f"Tool '{tool_config.name}' of type 'a2a_call' is missing 'agent_url' in its tool_schema."
                    )
                logger.debug(f"Tool '{tool_config.name}' validated successfully.")

        conn = self._get_conn()
        logger.debug(f"Executing INSERT OR REPLACE for agent_info with key = 1, name = {info.name}")
        conn.execute(
