import { ConfigFileSchema, transformConfigToAgentInfo, type ConfigFile } from './config-schema.ts';
    ...
    try {
      ...
      try {
        ...
        if (error instanceof Deno.errors.NotFound) {
          log.info(`Config file not found at ${filePath}, using database defaults`);
          return null;
        }
        throw error;
      }
      ...
      try {
        ...
        throw new Error(`Invalid JSON in config file ${filePath}: ${parseError}`);
      }

      // Validate against schema
      const validationResult = ConfigFileSchema.safeParse(parsedJson);
      if (!validationResult.success) {
        log.error('Config file validation failed:', validationResult.error.issues);
      ...
      log.info(`Successfully loaded and validated config file: ${configFile.agent.name}`);
