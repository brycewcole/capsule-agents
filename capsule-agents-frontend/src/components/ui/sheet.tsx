...
function SheetContent({ className, children, side = "right", hideCloseButton, container, withOverlay = true, ...props }: React.ComponentProps<typeof DialogPrimitive.Content> & {
  ...
  const sideClasses = {
    right: "inset-y-0 right-0 h-full w-[420px] max-w-[calc(100%-4rem)] border-l",
...
function SheetHeader({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div data-slot="sheet-header" className={cn("flex flex-col gap-2 p-4 border-b", className)} {...props} />
  )
}

function SheetFooter({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div data-slot="sheet-footer" className={cn("flex items-center gap-2 p-4 border-t", className)} {...props} />
  )
}

function SheetTitle({ className, ...props }: React.ComponentProps<typeof DialogPrimitive.Title>) {
  return (
    <DialogPrimitive.Title data-slot="sheet-title" className={cn("text-base font-semibold", className)} {...props} />
  )
}
